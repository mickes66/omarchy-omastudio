...
pub fn process_buffer_16_to_8(
    ...
    // Process rows in parallel with Rayon
    let in_stride = (width as usize) * ch;
    let out_stride = (width as usize) * ch;
    output
        .par_chunks_mut(out_stride)
        .enumerate()
        .for_each(|(y_idx, row)| {
            let in_row = &input[y_idx * in_stride..(y_idx + 1) * in_stride];
            let y = y_idx as u32;

            for x in 0..width {
                let px = (x as usize) * ch;

                let r_norm = (in_row[px] as f32 / 65535.0) * wb_r;
                let g_norm = (in_row[px + 1] as f32 / 65535.0) * wb_g;
                let b_norm = (in_row[px + 2] as f32 / 65535.0) * wb_b;
    ...
    output
        ...
        .for_each(|(y_idx, row)| {
            ...
            for x in 0..width {
                ...
                row[px] = (r5.clamp(0.0, 1.0) * 65535.0) as u16;
                row[px + 1] = (g5.clamp(0.0, 1.0) * 65535.0) as u16;
                row[px + 2] = (b5.clamp(0.0, 1.0) * 65535.0) as u16;
        ...
        .for_each(|(y_idx, row)| {
            ...
            for x in 0..width {
                ...
                if x < split_x {
                    ...
                    row[px] = (in_row[px] >> 8) as u8;
                    row[px + 1] = (in_row[px + 1] >> 8) as u8;
                    row[px + 2] = (in_row[px + 2] >> 8) as u8;
                } else if x == split_x || x == split_x + 1 {
                    // White split separator line
                    row[px] = 255;
                    row[px + 1] = 255;
